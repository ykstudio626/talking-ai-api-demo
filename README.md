...
### OpenAI Realtime API
現時点で最高性能と言われている「リアルタイム会話用API」です。
https://developers.openai.com/api/docs/guides/realtime-conversations#handling-audio-with-webrtc

裏でGPT-4o-mini相当のモデルが動いているそうですが、リアルタイムの応答に特化しており、レスポンスが速いです（おそらく音声を逐次解析している）。

書き起こし（STT）と読み上げ（TTS）のフェーズを挟まず、全自動で応答が行われるのが特徴。
（テキストは別チャネルで取得可能、本コードも取得したテキストを画面に逐次表示しています）
...
- vite.config.tsはローカルでの実行用のファイルで、VercelのSeverless環境をエミュレートしています。本番ではsession.tsが走ります。
