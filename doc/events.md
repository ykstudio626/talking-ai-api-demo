...
response / response.*: AI の「応答」単位（1 回の発話・出力）のライフサイクルを表します。response.created（開始）→ response.audio_transcript.delta（逐次テキスト）→ response.done（終了）などのイベントがあり、msg.response.id で応答インスタンスを追跡します。音声は WebRTC のリモートトラックで届き、テキスト（逐次文字起こし）は DataChannel の response.audio_transcript.* 系で届きます。
